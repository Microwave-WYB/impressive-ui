...
from impressive_ui.gtk import MutableState, State, Preview
from impressive import apply, attempt
...
class CalculatorModel:
    ...
    def sync_result(self) -> "CalculatorModel":
        return (
            attempt(
                lambda: replace(
                    self, result=str(eval(self.current_expression)), error=False
                ),
            )
            .catch(SyntaxError)
            .fallback(self)
            ...
            case Control.DECIMAL:
                match self.current_expression[-1]:
                    ...
                    case operator if not operator.isdigit():
                        return replace(
                            self, current_expression=self.current_expression + "0."
                        )
                    case _:
                        return replace(
...
def Keypad(view_model: CalculatorViewModel) -> Gtk.Grid:
    ...
    def _():
        return (
            ...
            (
                CalcButton(
                    "C", lambda: view_model.enter(Control.CLEAR), ["destructive-action"]
                ),
                0,
                0,
                3,
            ...
            (
                CalcButton(
                    "edit-clear-symbolic",
                    lambda: view_model.enter(Control.BACKSPACE),
                    ["flat"],
                    icon=True,
                ),
                3,
            ...
            (
                CalcButton(
                    "÷", lambda: view_model.enter(Operator("/")), ["suggested-action"]
                ),
                3,
                1,
                1,
            ...
            (
                CalcButton(
                    "=", lambda: view_model.enter(Control.EVAL), ["suggested-action"]
                ),
                0,
                5,
                4,
...
if __name__ == "__main__":
    ...
    def _(_) -> Gtk.Widget:
        view_model = CalculatorViewModel()
        view_model._state.set(
            CalculatorModel(current_expression="2+3*4", result="14", error=False)
        )
    ...
    def _(_) -> Gtk.Widget:
        view_model = CalculatorViewModel()
        view_model._state.set(
            CalculatorModel(current_expression="2+3*", result="Error!", error=True)
        )
