...
from impressive_ui.gtk.state import MutableState, State
...
def ErrorWidget(error_message: str) -> Gtk.Widget:
...
def WindowLauncher(
    ...
    def _():
        ...
        def _(*_):
            window = widget_factory(event_loop)
    ...
    try:
        ...
        if isinstance(widget, Gtk.Window):
            ...
            return WindowLauncher(
                name or widget_factory.__name__, widget_factory, event_loop
            )

        return widget

    except Exception as e:
        return ErrorWidget(f"Error creating widget: {str(e)}")
...
class PreviewViewModel:
    ...
    def __init__(self, widgets: dict[str, Callable]):
        ...
        # State properties
        self._selected_widget = MutableState(
            self._widget_names[0] if self._widget_names else ""
        )
    ...
    def get_widgets(
        self,
    ) -> dict[str, Callable[[asyncio.AbstractEventLoop], Gtk.Widget]]:
        """Get the widgets dictionary for external factory functions."""
...
def PreviewArea(
    view_model: PreviewViewModel, event_loop: asyncio.AbstractEventLoop
) -> Gtk.Widget:
...
def _update_preview_content(
    ...
    if widget_name and view_model.has_widgets:
        widgets = view_model.get_widgets()
        if widget_name in widgets:
            widget_factory = widgets[widget_name]
            preview_widget = PreviewTarget(widget_factory, event_loop, widget_name)
            if preview_widget:
