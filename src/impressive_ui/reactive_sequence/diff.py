...
def compute_diff_operations(
    ...
    moved_keys = {
        key
        for key in old_key_to_index
        if key in new_key_to_index and key not in items_to_keep
    }
...
def diff_update(
    ...
    for op in compute_diff_operations(
        old_key_to_index, new_key_to_index, new_source, key_func
    ):
        apply_operation(op)
